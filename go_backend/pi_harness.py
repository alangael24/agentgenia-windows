...
class PiHarness:
    def __init__(
        ...
        *,
        ...
        node_run_as_electron: bool = False,
        ...
        self.node_run_as_electron = node_run_as_electron
    ...
    def _resolved_binary(self) -> str | None:
        if os.path.sep in self.binary:
            ...
            is_script = path.suffix.lower() in (".js", ".mjs", ".cjs")
            return str(path) if path.is_file() and (is_script or os.access(path, os.X_OK)) else None
        return shutil.which(self.binary)

    def _resolved_node_binary(self) -> str | None:
        if self.node_binary:
            configured = Path(self.node_binary).expanduser()
            if configured.is_file() and os.access(configured, os.X_OK):
                return str(configured)
            resolved = shutil.which(self.node_binary)
            if resolved:
                return resolved
        runtime_path = os.environ.get("PATH", os.defpath)
        if self.node_bin_dir:
            runtime_path = self.node_bin_dir + os.pathsep + runtime_path
        return shutil.which("node", path=runtime_path)
        ...
        if os.name == "nt":
            for env_name in ("LOCALAPPDATA", "PROGRAMFILES", "PROGRAMFILES(X86)"):
                ...
                if not root:
                    continue
                candidates.extend((
                    str(Path(root) / "Google" / "Chrome for Testing" / "Application" / "chrome.exe"),
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "node_available": bool(self._resolved_node_binary()),
            "browser_available": self._browser_available(),
            "browser_extension_installed": bool(self._resolved_chrome_extension()),
    ...
    def _child_env(
        ...
        self._inherit_windows_runtime_env(env)
    ...
    @staticmethod
    def _inherit_windows_runtime_env(env: dict[str, str]) -> None:
        """Hereda solo variables de sistema necesarias, nunca secretos del backend."""
        if os.name != "nt":
            return
        for name in (
            ...
            "SYSTEMDRIVE", "SYSTEMROOT", "USERDOMAIN", "USERNAME",
            "USERPROFILE", "WINDIR",
        ):
